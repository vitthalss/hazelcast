...
public class PlanCacheIntegrationTest extends PlanCacheTestSupport {
    ...
    @Test
    public void testPlanInvalidationOnShadowing() {
        HazelcastInstance member = factory.newHazelcastInstance();

        ReplicatedMap<Integer, Integer> replicatedMap = member.getReplicatedMap("map");
        ...
        IMap<Integer, Integer> partitionedMap = member.getMap("map");
        partitionedMap.put(1, 1);
    ...
    public void testPlanInvalidationOnExceptionWithInvalidationFlag() {
        ...
        setExecHook(member, exec -> {
            if (exec instanceof MapScanExec) {
               exec = new FaultyExec(exec, QueryException.error("Failed").withInvalidate());
            }
